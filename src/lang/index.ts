...
export async function loadLang (app: App) {
  const { lang } = appStore.useGlobalSetting
  const locale = lang.defaultLang

  // 加载框架语言包
  const langs = await import(`./globs-${locale}.ts`)
  const message = langs.default ?? {}
  ...
  i18n = createI18n({
    ...
    fallbackLocale: lang.fallbackLang,
    messages
  })
      ...
      if (pathName.indexOf('/') > 0) {
        ...
        if (pathNameTmp.length === 2) {
          ...
          msg[pathNameTmp[0]][pathNameTmp[1]] =
            handleMsglist(mList[path].default)
          ...
          msg[pathNameTmp[0]][pathNameTmp[1]][pathNameTmp[2]] =
            handleMsglist(mList[path].default)
        } else {
...
export function editDefaultLang (lang: string): void {
  const { setLang } = appStore.useGlobalSetting
  setLang(lang)
